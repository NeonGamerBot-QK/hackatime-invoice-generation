...
; (async () => {
  ...
  // Load heartbeats
  const heartbeats = await fetch("https://hackatime.hackclub.com/api/admin/v1/execute", {
    method: "POST",
    headers: { Authorization: `Bearer ${process.env.HACKATIME_ADMIN_KEY}`, "Content-Type": "application/json" },
    // IF U HAVE MORE THEN 9999 WHY 
    body: JSON.stringify({ query: `select raw_data from heartbeats where user_id = ${process.env.HACKATIME_ID} AND project = '${process.env.HACKATIME_PROJECT}' LIMIT 99999;` }),
  ...
  // Save
  fs.writeFileSync('./invoice_ai.csv', `date,total_seconds,hours,pay\n${rows.join('\n')}`);
